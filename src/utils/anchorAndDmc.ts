...
import { AnchorAndDmcType } from "../typings"
...
export const getAnchorAndDmcList = () => {
    ...
    anchorList.forEach((anchor) => {
        if (!usedAnchor.get(anchor))
            anchorAndDmc.add(JSON.stringify({ anchor: anchor, dmc: 'N/A' }))
    })


    dmcList.forEach((dmc) => {
        if (!usedAnchor.get(dmc))
            anchorAndDmc.add(JSON.stringify({ anchor: 'N/A', dmc: dmc }))
    })

    const anchorAndDmcArray = [...anchorAndDmc.values()]
    const anchorAndDmcComplete = anchorAndDmcArray.map((value): AnchorAndDmcType =>
        JSON.parse(value))
