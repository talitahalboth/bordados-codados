...
import type { AnchorAndCirculoType } from '../typings'
...
export const getAnchorAndCirculoList = () => {
  ...
  anchorToCirculo.forEach((obj) => {
    ...
    anchorAndCirculo.add(
      JSON.stringify({ anchor: obj.anchor, circulo: obj.circulo })
    )
  })

  anchorList.forEach((anchor) => {
    if (!usedAnchor.get(anchor))
      anchorAndCirculo.add(JSON.stringify({ anchor, circulo: 'N/A' }))
  })

  circuloList.forEach((circulo) => {
    if (!usedAnchor.get(circulo))
      anchorAndCirculo.add(JSON.stringify({ anchor: 'N/A', circulo }))
  })

  const anchorAndCirculoArray = [...anchorAndCirculo.values()]
  const anchorAndCirculoComplete = anchorAndCirculoArray.map(
    (value): AnchorAndCirculoType => JSON.parse(value)
  )
