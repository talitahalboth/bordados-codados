...
import { DmcAndCirculoType } from "../typings"
...
export const getCirculoAndDMcList = () => {
    ...
    dmcList.forEach((dmc) => {
        if (!usedDmc.get(dmc))
            dmcAndCirculo.add(JSON.stringify({ circulo: 'N/A', dmc: dmc }))
    })


    circuloList.forEach((circulo) => {
        if (!usedDmc.get(circulo))
            dmcAndCirculo.add(JSON.stringify({ circulo: circulo, dmc: 'N/A' }))
    })

    const dmcAndCirculoArray = [...dmcAndCirculo.values()]
    const dmcAndCirculoComplete = dmcAndCirculoArray.map((value): DmcAndCirculoType =>
        JSON.parse(value))

    const sorted = dmcAndCirculoComplete.sort((a, b) => {
        if (isNumeric(a.circulo[0]) && isNumeric(b.circulo[0])) {
            if (a.circulo.length === b.circulo.length) return a.circulo.localeCompare(b.circulo)
